...
MERGE (reynaud)-[:SAYS]->(conv2db3)-[:TO]->(jaymes)
...
MERGE (conv2db4)-[:NEXT]->(conv2db5:DialogBlock {sentenceCnt:3, wordsPerSentence:[3,10,6], toneList:["questioning","suspitious"]})
MERGE (reynaud)-[:SAYS]->(conv2db5)-[:TO]->(jaymes)
MERGE (conv2db5)-[:NEXT]->(conv2db6:DialogBlock {sentenceCnt:1, wordsPerSentence:[1], toneList:["neutral"]})
MERGE (jaymes)-[:SAYS]->(conv2db6)-[:TO]->(reynaud)
MERGE (conv2db6)-[:NEXT]->(conv2db7:DialogBlock {sentenceCnt:1, wordsPerSentence:[5], toneList:["neutral"]})
MERGE (dram)-[:SAYS]->(conv2db7)-[:TO]->(reynaud)
